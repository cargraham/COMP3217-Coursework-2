...
pricing_guidelines = pd.read_csv("TestingResults.txt", header=None)
pricing_guidelines.insert(loc=0, column='row_num', value=pricing_guidelines.index + 1)

#filter the pricing guidelines so that only the abnormal ones remain
is_abnormal = pricing_guidelines.iloc[:, -1] == 1
pricing_guidelines = pricing_guidelines[is_abnormal]
